...
def foward_detector_roifeature(model, images, targets=None, return_maps=False,saveFolder='',imgnnames=None):
    ...
    for fea in features:
        ...
        scales.append(cw/ow)
    pooler = Pooler(
        output_size=output_size,#(7, 7),
        scales=scales,
        sampling_ratio=0,
    )
    pooledfeature=pooler.extract(features,targets)
...
def foward_detector_roifeature_one(model, images, targets=None, return_maps=False,saveFolder='',imgnnames=None):
    ...
    p0 = pooledfeature[0].cpu().detach().numpy()  # for p0
    for i in range(len(pooledfeature)):
        pi=pooledfeature[i].cpu().detach().numpy()#for p0
    ...
    #imgind=4
    cfeamap=pooler.extract(score_maps['centerness'], targets)
